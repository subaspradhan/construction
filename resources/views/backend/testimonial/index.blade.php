...
<div class="dashboard-main-body">
      ...
      <div class="d-flex flex-wrap align-items-center justify-content-between gap-3 mb-24">
        <h6 class="fw-semibold mb-0">Testimonial Table</h6>
      </div>

      <div class="card basic-data-table">
        <div class="d-flex justify-content-end align-items-center p-3">
          <a href="/testi-create" class="btn btn-primary">
              + Add Data
          </a>
        </div>
        ...
        <div class="table-responsive card-body">
          <table class="table bordered-table mb-0" id="dataTable" data-page-length='10'>
            ...
            <tbody>
            @if ($testi->count() > 0)
            @foreach ($testi as $key => $testis)
              <tr>
                <td>{{ $key + 1 }}</td>
                <td>{{ $testis->name}}</td>
                <td>{{ $testis->designation }}</td>
                
                <td><img src="{{ asset('storage/' . $testis->image) }}"
                alt="testis Image" style="width: 100px;"></td>
                <td>{{$testis->message}}</td>
                <td>
                  <a href="{{ route('testi.admin.edit', $testis->id) }}"
          ...
          <div class="mt-5">{{ $testi->links('pagination::bootstrap-5') }}</div>
        </div>
      </div>
    </div>
    <script>
      document.addEventListener("DOMContentLoaded", function () {
        ...
        deleteButtons.forEach(button => {
          button.addEventListener('click', function () {
            ...
            const deleteUrl = "{{ url('/testi-delete') }}/" + id;
